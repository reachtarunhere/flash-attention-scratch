...
def _flash_attention_forward_causal_kernel(
    ...
    for start_n in range(0, q_block_idx * BLOCK_M, BLOCK_N):
        ...
        k_batch_head_base_ptr = K_ptr + batch_idx * k_stride_b + head_idx * k_stride_h
        # Above basically gets us the ptr to K[batch_idx, head_idx, 0, 0]
        # now we do the same for v
        v_batch_head_base_ptr = V_ptr + batch_idx * v_stride_b + head_idx * v_stride_h
...
def flash_attention_forward(q, k, v, is_causal=True):
    ...
    if not is_causal:
        raise NotImplementedError(
            "This implementation is for causal attention. Use solution_3 for non-causal."
        )
